#!/usr/bin/env python3
"""M2P Layer Depth Scaling to L=36: Option A only, sweep L={16,24,36}.

TYPE: frontier-extension (Type 3)
MATH: experiments/models/m2p_layer_depth_36/MATH.md
...
PRIOR FINDINGS:
  ...
  Finding #363 (exp_m2p_layer_depth):   L={2,4,8,16}, Option A quality=86.4% at L=16
...
QUESTION:
  ...
  L=36 is the first test beyond the rank-saturation boundary where L x LORA_RANK > d_M2P:
...
MATHEMATICAL FRAMEWORK:
  ...
  Theorem B.3 (MATH.md): Aghajanyan intrinsic dimensionality model predicts quality
    plateau at 83-90% once shared cross-layer structure saturates the bottleneck.

KILL CRITERIA:
  K894: Option A quality_ratio >= 85% at L=36 (PASS criterion)
  K895: Max train-val gap < 0.7 nats at L=36 (PASS criterion)
  K896 (KILL): Option A quality_ratio < 50% at L=16 (kill trigger)
...
ARCHITECTURE FIXED (same as proven recipe):
  ...
  Domains: sort + reverse + arithmetic (all 3 included; parity guard decides exclusion)

SWEEP: n_layers in {16, 24, 36}
OUTPUT: results.json with option_a_quality_ratio_{L} for each L.
"""
...
LAYER_VALUES = [16, 24, 36] if not SMOKE_TEST else [16, 24]
...
def phase_depth_sweep(domain_data: dict) -> dict:
    ...
    for n_layers in LAYER_VALUES:
        ...
        # Kill criteria (match MATH.md/PAPER.md):
        # K894: Option A quality >= 85% at L=36 (PASS criterion)
        # K895: Max train-val gap < 0.7 nats at L=36 (PASS criterion)
        # K896 (KILL): Option A quality < 50% at L=16 (kill trigger)
        k894_pass = (optA_q >= 0.85) if n_layers == 36 else None
        k895_pass = (optA_agg["max_train_val_gap"] < 0.7) if n_layers == 36 else None
        k896_kill = (optA_q < 0.50) if n_layers == 16 else None

        if k896_kill is not None and k896_kill:
            log(f"  *** K896 KILL TRIGGERED: Option A quality < 50% at L=16 ***")
            log(f"  *** M2P cannot serve deep models at d_M2P=64 ***")
        if k894_pass is not None:
            log(f"  K894 (L=36 Option A >= 85%): {'PASS' if k894_pass else 'FAIL'}")
        if k895_pass is not None:
            log(f"  K895 (L=36 train-val gap < 0.7): {'PASS' if k895_pass else 'FAIL'}")
        ...
        # EARLY EXIT if K896 triggered (Option A killed at L=16)
        if k896_kill:
            log(f"\n  K896 TRIGGERED at L={n_layers}. Stopping sweep early.")
            log(f"  M2P cannot serve deep models at d_M2P=64. Skipping remaining L.")
            break
...
def evaluate_kill_criteria(sweep_results: dict) -> dict:
    """Evaluate K894, K895, K896 from sweep results.
    ...
    # K896 (KILL) at L=16
    k896_result = None
    if "L16" in sweep_results and sweep_results["L16"]["k896_kill"] is not None:
        k896_result = sweep_results["L16"]["k896_kill"]
        log(f"\n  K896 (KILL): Option A < 50% at L=16: {'KILL' if k896_result else 'NOT TRIGGERED'}")

    # K894 at L=36
    k894_result = None
    if "L36" in sweep_results:
        k894_result = sweep_results["L36"]["k894_pass"]
        if k894_result is not None:
            log(f"  K894: Option A >= 85% at L=36: {'PASS' if k894_result else 'FAIL'}")

    # K895 at L=36
    k895_result = None
    if "L36" in sweep_results:
        k895_result = sweep_results["L36"]["k895_pass"]
        if k895_result is not None:
            log(f"  K895: Train-val gap < 0.7 nats at L=36: {'PASS' if k895_result else 'FAIL'}")

    # Overall interpretation
    if k896_result:
        outcome = "KILL_option_a_bottleneck_at_L16"
        interpretation = (
            "K896 TRIGGERED: Option A quality < 50% at L=16. "
            "M2P cannot serve deep models at d_M2P=64. "
            "Mitigation: increase d_M2P to 128 and retry. "
            "If that also fails, M2P needs per-layer-group calls."
        )
    elif k894_result and k895_result:
        outcome = "PASS_option_a_works_at_L36"
        interpretation = (
            "K894 PASS: Option A >= 85% at L=36. K895 PASS: gap < 0.7 nats. "
            "M2P single-call adapter generation scales to Qwen3-4B depth. "
            "Next: test on actual Qwen3-4B (Level 3/4 in PoC roadmap)."
        )
    elif k894_result is None:
        outcome = "INCOMPLETE_sweep_not_reached_L36"
        interpretation = (
            "Sweep did not reach L=36 (smoke test, early kill, or timeout). "
            "Check partial results."
        )
    elif k894_result and not k895_result:
        outcome = "PARTIAL_quality_ok_but_overfitting"
        interpretation = (
            "K894 PASS but K895 FAIL: Option A quality OK at L=36 but train-val gap "
            "exceeds 0.7 nats. Quality may be unreliable."
        )
    elif not k894_result:
        outcome = "FAIL_L36_below_threshold"
        interpretation = (
            "K894 FAIL: Option A < 85% at L=36. "
            "Compression cliff at L=36. "
            "Mitigation: increase d_M2P to 128 or use per-layer-group calls."
        )
    else:
        outcome = "FAIL_unexpected"
        interpretation = (
            "Unexpected combination of kill criteria results. "
            "Check if the experiment implementation matches MATH.md."
        )
    ...
    return {
        "k894_pass": k894_result,
        "k895_pass": k895_result,
        "k896_kill": k896_result,
        "outcome": outcome,
        ...
        "option_a_quality_L24": (summary.get("L24") or {}).get("option_a_quality"),
        "option_a_quality_L36": (summary.get("L36") or {}).get("option_a_quality"),
        "option_b_quality_L16": (summary.get("L16") or {}).get("option_b_quality"),
        "option_b_quality_L24": (summary.get("L24") or {}).get("option_b_quality"),
        "option_b_quality_L36": (summary.get("L36") or {}).get("option_b_quality"),
        "train_val_gap_optA_L16": (summary.get("L16") or {}).get("max_train_val_gap"),
        "train_val_gap_optA_L24": (summary.get("L24") or {}).get("max_train_val_gap"),
        "train_val_gap_optA_L36": (summary.get("L36") or {}).get("max_train_val_gap"),
    }
...
def main():
    ...
    log(f"")
    log(f"Kill criteria:")
    log(f"  K894 (Option A >= 85% at L=36): {kill_criteria.get('k894_pass')}")
    log(f"  K895 (train-val gap < 0.7 at L=36): {kill_criteria.get('k895_pass')}")
    log(f"  K896 KILL (Option A < 50% at L=16): {kill_criteria.get('k896_kill')}")
