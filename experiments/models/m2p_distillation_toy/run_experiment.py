...
def main():
    ...
    def m2p_loss(m2p_model, base_model, tokens, domain_id):
        """M2P loss: generate adapter from context, apply, measure task loss.

        Forward pass mirrors SFT GrassmannianLoRA exactly:
          - wq/wk/wv corrections applied INSIDE attention (before softmax)
          - wo correction applied to attention context (before output projection)
          - fc1 correction applied INSIDE MLP (before relu)
        This matches GrassmannianLoRA.__call__ which does base(x) + (x@A)@B*scale.
        """
        ...
        for li, layer in enumerate(base_model.layers):
            ...
            # LoRA-modified q, k, v — matches SFT: q = wq(x) + (x@A_wq)@B_wq*scale
            ...
            # LoRA-modified wo — matches SFT: wo(ctx) + (ctx@A_wo)@B_wo*scale
            attn_out = attn.wo(attn_ctx) + scale * (attn_ctx @ A_matrices[(domain_id, li, 3)]) @ all_B[li][3]
            x = x + attn_out

            # LoRA-modified fc1 INSIDE relu — matches SFT: fc2(relu(fc1(x) + (x@A)@B*scale))
            x_norm2 = layer.n2(x)
    ...
    m2p_losses = {}
    for di, name in enumerate(DOMAIN_NAMES):
        # Generate adapter from domain context
        ...
        # Evaluate with M2P adapter (using same simplified forward)
        total = 0; n = 0
        for tokens in domain_data[name]["val"][:30]:
            loss = m2p_loss(m2p, base, tokens, di)
            mx.eval(loss)
            total += loss.item(); n += 1
    ...
    results = {
        "experiment": "m2p_distillation_toy",
