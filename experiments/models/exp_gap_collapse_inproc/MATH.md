...
Identify a pair of experts whose interference gap $g_{AB}$ on a target concept is purely destructive (e.g., interfering math specialists where composition degrades accuracy); if collapsing this gap results in **behavioral reinforcement** (improved performance) rather than the predicted unlearning, the causal link is broken. The hypothesis is killed if the interference gap is zeroed while the target concept score drops by less than 50% or OOD A/B separation degrades by less than 5pp.
