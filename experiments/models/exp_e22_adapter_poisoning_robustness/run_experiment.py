...
Tests whether Grassmannian orthogonality limits contamination from a
poisoned adapter during composition, compared to random (non-orthogonal)
A matrices.

Phases:
1. Load model, create clean + poisoned adapters (Grassmannian and random A)
2. Compose adapters into base weights under each condition
3. Generate answers to knowledge QA questions
4. Score accuracy and measure degradation
"""
...
def compose_adapters(W_base, As, Bs):
    """Compose: W_composed = W_base + Σ B_i @ A_i"""
    delta = mx.zeros_like(W_base)
    for A_i, B_i in zip(As, Bs):
        delta = delta + B_i @ A_i
    mx.eval(delta)
    return W_base + delta
...
def evaluate_qa(model, tokenizer, n_qa):
    """Run QA evaluation, return accuracy."""
    pairs = QA_PAIRS[:n_qa]
    correct = 0
...
def apply_composition_to_model(model, layer_indices, compositions):
    """Apply pre-computed ΔW to model's v_proj weights in-place."""
...
def run_experiment():
    POISON_MULTIPLIERS = [1, 5, 10, 20] if SMOKE_TEST else [1, 3, 5, 10, 15, 20]

    print("=" * 60)
    print("E22: Adapter Poisoning Robustness")
    ...
    for layer_idx in LAYER_INDICES:
        ...
        # Base W Frobenius norm for reference
    ...
    # Phase 4: Sweep poison multipliers
    print("\n--- Phase 4: Poison magnitude sweep ---")

    # Evaluate clean-only first
    ...
    for mult in POISON_MULTIPLIERS:
        print(f"\n  --- Poison multiplier: {mult}× ---")

        # Create poisoned deltas at this magnitude
        grass_poison_deltas = {}
        rand_poison_deltas = {}
        ...
        print(f"  Protection margin:  {margin:.1f}pp")
    ...
    # Phase 5: Verdict using worst-case (highest multiplier)
    print("\n--- Phase 5: Analysis ---")
    ...
    for r in sweep_results:
        print(f"  {r['multiplier']:3d}×: grass={r['grassmannian_poison_accuracy']:.1f}% "
              f"rand={r['random_poison_accuracy']:.1f}% "
              f"margin={r['protection_margin_pp']:.1f}pp")

    # K2055: Use highest multiplier — does Grassmannian avoid catastrophic drop?
    k2055_pass = worst_grass_drop < 30.0
    # K2056: At ANY multiplier, does Grassmannian show >2pp protection?
    k2056_pass = best_margin > 2.0

    all_pass = k2055_pass and k2056_pass

    if all_pass:
        verdict = "SUPPORTED" if not SMOKE_TEST else "PROVISIONAL"
    else:
        verdict = "KILLED"

    print(f"\nK2055 (worst grass drop < 30pp): {'PASS' if k2055_pass else 'FAIL'} ({worst_grass_drop:.1f}pp)")
    print(f"K2056 (best protection margin > 2pp): {'PASS' if k2056_pass else 'FAIL'} ({best_margin:.1f}pp)")
    print(f"Verdict: {verdict}")

    results_out = {
        "experiment": "E22",
        "title": "Adapter Poisoning Robustness — Grassmannian as safety feature",
        "model": MODEL_ID,
        "is_smoke": SMOKE_TEST,
        ...
        "kill_criteria": {
            "K2055": {
                "description": "Grassmannian composition degrades catastrophically with poisoned adapter (> 30pp drop)",
                "threshold": "< 30pp drop from base at worst multiplier",
                "measured": f"{worst_grass_drop:.1f}pp",
                "pass": k2055_pass,
            },
            "K2056": {
                "description": "Orthogonality provides no protection vs naive averaging",
                "threshold": "> 2pp protection margin at best multiplier",
