...
class M2PTransformerConditioned(nn.Module):
    """Domain-conditioned M2P Transformer.

    Identical to M2PTransformer (rev1) except:
      - domain_embed: nn.Embedding(N_DOMAINS, D_MODEL) provides explicit domain signal
      - __call__ receives domain_id (int 0..N-1) and injects embed into memory tokens

    Architecture:
      Input: hidden states from base model (L, M, H) + domain_id (scalar int)
      Output: B-matrices for LoRA adapters
    """
    def __init__(self, d, n_layers=2, n_heads=4, n_target_modules=5):
        super().__init__()
        # Domain embedding: the key addition (Theorem 3)
        # N_DOMAINS × D_MODEL = 5 × 64 = 320 parameters
        self.domain_embed = nn.Embedding(N_DOMAINS, d)
    ...
    def __call__(self, hidden_states_list, domain_id):
        ...
        domain_id: int, domain index 0..N_DOMAINS-1

        Returns: list of L dicts, each mapping module_index → B matrix (rank, out_dim)
        """
        # Extract memory: mean-pool each layer's hidden states
        ...
        # Add learnable memory embed and positional encodings
        memory = memory + mx.broadcast_to(self.memory_embed, memory.shape)
        memory = memory + self.p_layer + self.p_token

        # === Domain conditioning (Theorem 3, MATH.md) ===
        # Inject domain embedding additively into ALL memory tokens across ALL layers
        # Gradient: ∂L_d/∂e_d is domain-private → embeddings diverge → B-matrices differentiate
        # domain_embed(domain_id): (D_MODEL,) → broadcast to (L, M, D_MODEL)
        d_emb = self.domain_embed(mx.array(domain_id))  # (D_MODEL,)
        d_emb_broadcast = mx.broadcast_to(d_emb[None, None, :], memory.shape)
        memory = memory + d_emb_broadcast

        # Run M2P blocks (unchanged from baseline)
...
def phase_train_m2p(base, A_matrices, domain_data):
    """Train domain-conditioned M2P. Returns trained m2p model."""
    log("\n=== Phase 4: Train Domain-Conditioned M2P ===")
    m2p = M2PTransformerConditioned(D_MODEL, n_layers=M2P_LAYERS, n_heads=N_HEADS,
    ...
    m2p_lg = nn.value_and_grad(m2p, m2p_loss)
...
def main():
    ...
    # Phase 4: Train domain-conditioned M2P
    m2p, m2p_loss_fn, m2p_params = phase_train_m2p(base, A_matrices, domain_data)
    ...
    results = {
        "experiment": "m2p_domain_conditioned",
