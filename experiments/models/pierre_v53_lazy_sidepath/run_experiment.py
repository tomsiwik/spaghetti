#!/usr/bin/env python3
"""Pierre v5.3: lazy bf16 LoRA side-path.

v5 overhead was 45% from 420 BitLinear.__call__ evals per forward pass.
v5.3 uses raw bf16 matmul — lazy graph, single eval at output.

Kill criteria:
  K736: Speed < 100 tok/s
  K737: Behavioral < 0.30
  K738: Memory > 8GB peak
"""
...
from pierre.v53 import (
    calibrate_router, route, inject_lazy_lora,
...
def phase_ppl():
    ...
    for di, d in enumerate(DOMAINS):
        ...
        n = inject_lazy_lora(model, skel, adapter, di, LORA_SCALE)
    ...
    for di, d in enumerate(DOMAINS):
        ...
        inject_lazy_lora(model, skel, adapter, ri, LORA_SCALE)
...
def phase_latency():
    ...
    inject_lazy_lora(model, skel, adapter, 0, LORA_SCALE)
    lazy_tps, lazy_mem = bench(model,tok,"lazy bf16 side-path")
...
def main():
    t0 = time.time()
    log("Pierre v5.3 — Lazy bf16 LoRA Side-Path")
    log("=" * 60)
    mx.random.seed(SEED)
    ...
    results = {
        "experiment": "pierre_v53_lazy_sidepath",
        "total_time_s": round(time.time()-t0, 1),
        "routing": r1, "ppl": r2, "behavioral": r3, "latency": r4,
        "kill_criteria": {
            "K736": {"pass": k1, "value": r4["lazy_tps"], "threshold": 100.0},
            "K737": {"pass": k2, "value": r3["overall"], "threshold": 0.30},
            "K738": {"pass": k3, "value": r4["lazy_peak_gb"], "threshold": 8.0},
        },
        ...
        "comparison": {
            ...
            "v53_lazy_sidepath": f"{r4['lazy_tps']} tok/s, {r4['overhead_pct']}% overhead",
