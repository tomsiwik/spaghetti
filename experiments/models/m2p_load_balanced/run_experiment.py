...
class M2PTransformerMoE(nn.Module):
    ...
    def __call__(self, hidden_states_list, domain_id, training=True):
...
def phase_train_m2p(base, A_matrices, domain_data, base_losses):
    ...
    def m2p_loss(m2p_model, base_model, tokens, domain_id, training=True):
        ...
        # Domain-conditioned B-matrix generation
        all_B = m2p_model(hidden_states, domain_id, training=training)
    ...
    def m2p_train_loss(m2p_model, base_model, tokens, domain_id):
        raw_loss = m2p_loss(m2p_model, base_model, tokens, domain_id, training=True)
        
        # M2P Loss Normalization
        name = DOMAIN_NAMES[domain_id]
        scale = base_losses[name]
        task_loss = raw_loss / scale

        # MoE Load Balancing Loss (Switch Transformer style)
        all_domain_logits = m2p_model.router.weight
        all_domain_probs = mx.softmax(all_domain_logits, axis=-1)
        expert_load = mx.mean(all_domain_probs, axis=0)  # (N_EXPERTS,)
        
        # Uniform target -> min sum of squares
        aux_loss = m2p_model.n_experts * mx.sum(mx.square(expert_load))
        aux_loss_coeff = 0.5
        
        return task_loss + (aux_loss_coeff * aux_loss)
...
def phase_evaluate_m2p(m2p, m2p_loss_fn, base, A_matrices, domain_data, base_losses, sft_losses):
    ...
    for di, name in enumerate(DOMAIN_NAMES):
        ...
        for tokens in domain_data[name]["val"][:30]:
            loss = m2p_loss_fn(m2p, base, tokens, di, training=False)
            mx.eval(loss)
            total += loss.item(); n += 1
...
def phase_composition_check(m2p, base, domain_data):
    ...
    for di, name in enumerate(DOMAIN_NAMES):
        ...
        all_B = m2p(hidden_states, di, training=False)
