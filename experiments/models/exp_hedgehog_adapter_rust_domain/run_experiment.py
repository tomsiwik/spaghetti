#!/usr/bin/env python3
"""
exp_hedgehog_adapter_rust_domain — Hedgehog per-layer cos-sim distillation for Rust
domain knowledge.

Pre-registered KCs (canonical DB text — do not edit):
  K#1866 proxy:  Rust adapter PPL on Rust-specific eval > base + generic LoRA
                 (kill if TRUE — Hedgehog strictly worse)
  K#1867 target: Rust idiomaticity auto-judge < +5pp vs base
                 (pair K1866 per F#666)  (kill if TRUE — target threshold not met)
...
Runtime structure (mirrors sibling exp_hedgehog_adapter_python_domain):
  Phase 0 — Rust Book + nomicon + RFC corpus + (Q, A) pair curation
  Phase A — teacher attention-output capture: 26B Gemma variant + π_Rs in context + Q
  ...
  Phase Baseline — generic token-space LoRA at matched params (for K1866 head-to-head)
  Phase C — K1866 PPL on held-out Rust eval (base vs base+gen-LoRA vs base+Hedgehog)
  Phase D — K1867 idiomaticity auto-judge on 50 blind-paired prompts (+ cargo check)
...
BATCH_SIZE = 1
SEQLEN = 2048                           # Rust Book + nomicon blocks run longer than snippets
ENABLE_THINKING = True                  # F#614 — load-bearing on Gemma 4 reasoning

RS_FOCUS_TOPICS = (
    "ownership_and_move_semantics",
    "borrow_checker_and_lifetimes",
    "traits_and_trait_objects",
    "iterators_and_closures",
    "unsafe_and_ffi",
    "error_handling_result_option_question_mark",
    "declarative_and_procedural_macros",
    "zero_cost_abstractions_and_raii",
)
...
def prepare_rs_qa_pairs(data_dir: Path, n_train: int, n_heldout: int) -> dict:
    """Curate (topic, π_Rs, Q, A) pairs for the 8 focus topics.

    NOT YET IMPLEMENTED. Requires:
      - Rust Book chapter excerpts (fair-use summaries) for each of RS_FOCUS_TOPICS.
      - Nomicon sections on unsafe/memory (fair-use, CC-BY-4.0 per rust-lang.github.io).
      - Selected RFC summaries (edition transitions, notable stabilizations).
      ...
      - External Rust eval corpus for K1866 PPL: top-N crates.io open-source samples
        (tokio/serde/clap/rayon function bodies), filtered for code-only, disjoint from
        Rust Book/nomicon to avoid train contamination.

    Load-bearing for K1866 (PPL eval text selection) and K1867 (judge reference answers
    + compile-check ground truth).
    """
    raise NotImplementedError(
        "Phase 0 Rust corpus + Q-A pair curation not yet implemented. "
        "Requires Rust Book + nomicon + RFC text + validated (Q, A) pairs per "
        f"{len(RS_FOCUS_TOPICS)} focus topics + external crates.io PPL eval corpus. "
...
# 4. Loss per batch:
#       L = mean_l (1 - cos(A_l_teacher(Q | π_Rs), A_l_student(Q)))
...
def train_generic_token_lora_baseline(data_dir: Path, adapter_path: Path) -> dict:
    """Baseline for K1866: generic token-space LoRA next-token CE on (Q -> A) pairs at
    matched rank/targets/scale/steps. This IS available via mlx_lm.lora CLI — but
    running it in isolation without the Hedgehog arm would produce an unpaired K1866
    measurement (PPL comparison loses its head-to-head anchor). Deferred to the _impl
    follow-up so both arms land together.
    """
    raise NotImplementedError(
        "Generic token-space LoRA baseline deferred to _impl follow-up to keep K1866 "
        "head-to-head PPL measurement paired with Hedgehog arm."
    )


# ─────────────────────────────────────────────
# Phase C: K1866 PPL head-to-head
# ─────────────────────────────────────────────

def measure_k1866_ppl(student_adapter: Path, baseline_adapter: Path,
                      rs_eval_text: list) -> dict:
    """K1866: PPL(base), PPL(base + generic token-space LoRA), PPL(base + Hedgehog).
    ...
    NOT YET IMPLEMENTED. Requires:
      ...
      - For each config, compute cross-entropy on tokenized held-out rs_eval_text
        (external crates.io slice, disjoint from Rust Book/nomicon training docs).
      - PPL = exp(mean_cross_entropy).
      - Report all three PPLs, plus the pairwise deltas needed for K1866.
    """
    raise NotImplementedError(
        "K1866 PPL head-to-head depends on both Hedgehog and baseline adapters existing."
    )


# ─────────────────────────────────────────────
# Phase D: K1867 idiomaticity auto-judge
# ─────────────────────────────────────────────

def measure_k1867_idiomaticity_judge(student_adapter: Path,
                                     held_out: list) -> float:
    """K1867: blind-paired judge scores idiomaticity of base vs Hedgehog-adapted answers
    on 50 held-out Rust-nuance prompts. Returns Δ = mean(score_hedgehog - score_base).
    ...
    NOT YET IMPLEMENTED. Requires:
      ...
      - judge rubric (0-10): (a) correct Rust feature usage, (b) idiomatic construct
        preference (iterator chains / ? operator / impl Trait / RAII), (c) borrow-
        checker correctness (cargo check compile-success when generation is a full
        function — hard floor: compile-fail → rubric correctness axis = 0).
      ...
      - Report mean(score_hedgehog - score_base) as a pp-delta on the 0–100 rescaled
        rubric.
    """
    raise NotImplementedError("K1867 judge eval depends on Hedgehog adapter existing.")
...
def main():
    ...
    print("Hedgehog Rust-domain adapter distillation (Gemma 4 E4B ← 26B)", flush=True)
    ...
    data_dir = EXPERIMENT_DIR / "data" / "rs_qa_pairs"
    adapters_dir = EXPERIMENT_DIR / "adapters"
    student_adapter = adapters_dir / "hedgehog_rs_r8"
    baseline_adapter = adapters_dir / "token_lora_rs_r8"
    ...
    results = {
        ...
        "rs_focus_topics": list(RS_FOCUS_TOPICS),
        ...
        "phase_d_k1867": {},
        "kc": {
            "K1866_hedgehog_ppl_not_greater_than_generic_lora": "untested",
            "K1867_idiomaticity_delta_ge_5pp": "untested",
        },
        "verdict": "PROVISIONAL",
        "all_pass": False,
        "blockers": [],
    ...
    # ── Phase 0: dataset ────────────────────────────────────
    print("\n=== Phase 0: Rust Book + nomicon + RFC corpus + Q-A curation ===", flush=True)
    try:
        results["phase_0_dataset"] = prepare_rs_qa_pairs(data_dir, N_TRAIN, N_HELDOUT)
    ...
    if student_adapter.exists() and baseline_adapter.exists():
        try:
            k1866 = measure_k1866_ppl(student_adapter, baseline_adapter, rs_eval_text=[])
            results["phase_c_k1866"] = k1866
        except NotImplementedError as exc:
            results["blockers"].append(f"K1866 not implemented: {exc}")
        except Exception as exc:
            results["blockers"].append(f"K1866 failed: {exc}")
    else:
    ...
    # ── Phase D: K1867 idiomaticity judge ───────────────────
    if student_adapter.exists():
        try:
            k1867 = measure_k1867_idiomaticity_judge(student_adapter, held_out=[])
            results["phase_d_k1867"]["K1867_idiomaticity_delta_pp"] = round(float(k1867), 2)
        except NotImplementedError as exc:
            results["blockers"].append(f"K1867 not implemented: {exc}")
        except Exception as exc:
            results["blockers"].append(f"K1867 failed: {exc}")
    else:
        results["blockers"].append(
    ...
    # When the _impl follow-up lands, populate kc dict from phase_c_k1866 + phase_d_k1867
