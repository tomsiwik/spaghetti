...
Retest of killed exp_competitive_benchmark with the per-domain scale fix
(Finding #217, #220). Original kill: uniform 1/N at s=20 degraded knowledge
domains. Fix: oracle top-1 routing + per-domain optimal scales.

Kill criteria:
  K1 (#640): Routed SOLE worse than base on ANY benchmark -> KILL
  K2 (#641): Routed SOLE worse than Gemma-2-2B on >= 4/6 benchmarks -> KILL
...
DOMAINS = list(DOMAIN_SCALES.keys())
...
# ============================================================================
# Pre-merge composition with per-domain scale + oracle routing
# ============================================================================
...
def load_adapter(domain):
    """Load a single domain adapter B matrices from disk."""
...
def premerge_single_adapter(model, skeleton, adapter, domain, scale):
    """Pre-merge a single adapter into model weights at given scale.

    W_new = W_base + scale * B^T @ A^T
    """
...
def restore_base_weights(model, base_weights):
    """Restore model to base weights (undo pre-merge)."""
...
def save_base_weights(model):
    """Save base weights for restoration after pre-merge."""
    base_weights = []
    for layer in model.model.layers:
        layer_w = {}
        for key in TARGET_KEYS:
...
def eval_mmlu_domain(model_name, model, tokenizer, questions, domain, format_fn):
    """Evaluate MMLU for a single domain."""
    if not questions:
...
def phase_eval_bitnet_base(gsm8k, mmlu):
    ...
    log(f"\n  [MMLU] Evaluating bitnet-base...")
    mmlu_results = {}
    for domain in DOMAINS:
        mmlu_results[domain] = eval_mmlu_domain(
            "bitnet-base", model, tokenizer, mmlu[domain], domain,
            format_mmlu_prompt_generic,
        )
...
def phase_eval_bitnet_routed(gsm8k, mmlu):
    """Evaluate BitNet-2B + routed SOLE (oracle top-1 with per-domain scales).

    For each MMLU domain: pre-merge the matching adapter at its optimal scale.
    For GSM8K: pre-merge the math adapter at s=20.
    """
    log("\n" + "=" * 70)
    log("EVALUATING: BitNet-2B-4T + ROUTED SOLE (oracle top-1, per-domain scales)")
    log("=" * 70)
    t0 = time.time()
    mx.reset_peak_memory()
    ...
    model = replace_bitlinear_with_linear(model)
    model.freeze()
    log_memory("bitnet-routed-loaded-base")
    ...
    # Save base weights for restoration between domains
    base_weights = save_base_weights(model)
    log_memory("bitnet-routed-adapters-loaded")

    # GSM8K: route to math adapter at s=20
    log("\n  [GSM8K] Routing to math adapter (scale=20.0)...")
    premerge_single_adapter(model, skeleton, adapters["math"], "math", DOMAIN_SCALES["math"])
    gsm8k_results = eval_gsm8k("bitnet-routed", model, tokenizer, gsm8k, format_gsm8k_prompt_generic)
    restore_base_weights(model, base_weights)

    # MMLU: route each domain to its matching adapter
    log(f"\n  [MMLU] Evaluating with oracle top-1 routing + per-domain scales...")
    mmlu_results = {}
    for domain in DOMAINS:
        scale = DOMAIN_SCALES[domain]
        log(f"\n  Routing MMLU-{domain} to {domain} adapter (scale={scale})...")
        premerge_single_adapter(model, skeleton, adapters[domain], domain, scale)
        mmlu_results[domain] = eval_mmlu_domain(
            f"bitnet-routed-{domain}", model, tokenizer, mmlu[domain], domain,
            format_mmlu_prompt_generic,
        )
        restore_base_weights(model, base_weights)
        gc.collect()
    ...
    peak_mem = mx.get_peak_memory() / 1e9
    elapsed = time.time() - t0
    log(f"\nBitNet ROUTED SOLE total: {elapsed:.1f}s, peak memory: {peak_mem:.2f}GB")
...
def phase_eval_competitor(model_id, model_name, gsm8k, mmlu, gsm8k_fmt, mmlu_fmt):
    ...
    for domain in DOMAINS:
        mmlu_results[domain] = eval_mmlu_domain(
            model_name, model, tokenizer, mmlu[domain], domain, mmlu_fmt,
        )
...
def analyze_results(all_results):
    """Compare all models and evaluate kill criteria."""
    log("\n" + "=" * 70)
    log("ANALYSIS")
    log("=" * 70)
    ...
    analysis = {"model_scores": model_scores, "kill_criteria": {}}

    # Compare with original competitive benchmark (uniform composition)
    if "bitnet_base" in model_scores and "bitnet_routed" in model_scores:
        log("\n  IMPROVEMENT vs ORIGINAL UNIFORM COMPOSITION:")
        original_uniform = {
            "gsm8k": 0.48, "mmlu_medical": 0.45, "mmlu_code": 0.45,
            "mmlu_math": 0.25, "mmlu_legal": 0.45, "mmlu_finance": 0.45,
        }
        routed = model_scores["bitnet_routed"]
        for bench in benchmarks:
            orig = original_uniform.get(bench, 0)
            new = routed.get(bench, 0)
            delta = new - orig
            log(f"    {bench:<15}: uniform={orig:.3f} -> routed={new:.3f} (delta={delta:+.3f})")

    # K1: Routed SOLE worse than base on ANY benchmark
    if "bitnet_routed" in model_scores and "bitnet_base" in model_scores:
        routed = model_scores["bitnet_routed"]
        base = model_scores["bitnet_base"]
        worse_any = [b for b in benchmarks if routed.get(b, 0) < base.get(b, 0)]
        k1_fail = len(worse_any) > 0
        analysis["kill_criteria"]["k1_640"] = {
            "result": "fail" if k1_fail else "pass",
            "worse_benchmarks": worse_any,
            "evidence": f"Routed SOLE worse than base on {len(worse_any)}/{len(benchmarks)}: {worse_any}",
            "details": {b: {"routed": routed.get(b, 0), "base": base.get(b, 0),
                           "delta": routed.get(b, 0) - base.get(b, 0)} for b in benchmarks},
        }
        log(f"\n  K1 (#640): Routed worse than base on {len(worse_any)}: {worse_any} -> {'KILL' if k1_fail else 'PASS'}")

    # K2: Routed SOLE worse than Gemma-2-2B on >= 4/6 benchmarks
    if "bitnet_routed" in model_scores and "gemma2_2b" in model_scores:
        routed = model_scores["bitnet_routed"]
        gemma = model_scores["gemma2_2b"]
        worse_count = sum(1 for b in benchmarks if routed.get(b, 0) < gemma.get(b, 0))
        k2_fail = worse_count >= 4
        analysis["kill_criteria"]["k2_641"] = {
            "result": "fail" if k2_fail else "pass",
            "worse_count": worse_count,
            "evidence": f"Routed SOLE worse on {worse_count}/{len(benchmarks)} vs Gemma-2-2B",
            "worse_benchmarks": [b for b in benchmarks if routed.get(b, 0) < gemma.get(b, 0)],
            "better_benchmarks": [b for b in benchmarks if routed.get(b, 0) > gemma.get(b, 0)],
        }
        log(f"  K2 (#641): Routed worse on {worse_count}/6 vs Gemma-2-2B -> {'KILL' if k2_fail else 'PASS'}")

    # Also compare with Qwen (informational, not kill criterion)
    if "bitnet_routed" in model_scores and "qwen25_3b" in model_scores:
        routed = model_scores["bitnet_routed"]
        qwen = model_scores["qwen25_3b"]
        worse_count = sum(1 for b in benchmarks if routed.get(b, 0) < qwen.get(b, 0))
        analysis["qwen_comparison"] = {
            "worse_count": worse_count,
            "worse_benchmarks": [b for b in benchmarks if routed.get(b, 0) < qwen.get(b, 0)],
            "better_benchmarks": [b for b in benchmarks if routed.get(b, 0) > qwen.get(b, 0)],
        }
        log(f"  (Info) Routed worse on {worse_count}/6 vs Qwen2.5-3B")
...
def main():
    ...
    # Phase 1: BitNet base (no adapters)
    all_results["bitnet_base"] = phase_eval_bitnet_base(gsm8k, mmlu)

    # Phase 2: BitNet + Routed SOLE (oracle top-1, per-domain scales)
    all_results["bitnet_routed"] = phase_eval_bitnet_routed(gsm8k, mmlu)

    # Phase 3: Gemma-2-2B (size-matched comparator)
    ...
    # Phase 4: Qwen2.5-3B (stretch comparator)
    ...
    output = {
        "experiment": "competitive_benchmark_routed",
        "description": "Routed composition with per-domain optimal scales (Finding #217, #220)",
        "models": {
            "bitnet_base": {"id": MODEL_ID_BITNET, "description": "BitNet-2B-4T base, no adapters"},
            "bitnet_routed": {"id": MODEL_ID_BITNET, "description": "BitNet-2B-4T + oracle top-1 routing + per-domain scales"},
        ...
        "params": {
            ...
            "routing": "oracle_top1",
            "composition": "pre-merge single adapter",
        },
        "timing": {
            "total_s": total_time,
