# MATH — `exp_jepa_adapter_residual_stream_impl`

## §0. Platform skill attestation (PLAN.md §1011/§1012, reviewer (m2) gate)

Skills invoked this iteration (researcher iter ~41, 2026-04-25):
- **`/mlx-dev`** — INVOKED. Knowledge cited in this scaffold:
  - `mx.eval(model.parameters(), loss)` at step boundaries (Phase B requirement).
  - `mx.clear_cache()` between teacher/student phase boundaries (F#673).
  - `nn.value_and_grad(model, loss_fn)` for gradient computation; not `.backward()`.
  ...
  - Slices create COPIES (not views — opposite of NumPy); `at[idx].add` for accumulating updates.
  - bfloat16 preferred on M-series; float64 is CPU-only.
- **`/fast-mlx`** — DEFERRED. Will be invoked at the start of Phase B implementation iteration. Phase A executed in this iteration uses `mlx_lm.lora` subprocess (no custom training loop authored here), so /fast-mlx optimization patterns are not yet load-bearing.

**Inheritance.** Sections §1-§7 below are inherited **verbatim** from parent `exp_jepa_adapter_residual_stream/MATH.md`. KC IDs (#1817-#1820, mapping to parent K#1766-K#1769) are pre-registered in DB and have not been modified between parent and this _impl iteration. KC text is canonical (DB-anchored).

**This _impl iteration's marginal contribution:**
1. **Phase A token-space LoRA r=16 baseline** — actually executed here (parent deferred; cap).
2. **Phase B/C JEPA training loop** — still raises `NotImplementedError`; routed to a follow-up implementation iteration with /fast-mlx invocation budgeted.

---

# MATH — `exp_jepa_adapter_residual_stream` (parent inherited verbatim)
