...
## Sharpest falsification angle (gemini)
The sharpest way to falsify the **Conserved Gap Collapse** hypothesis is to identify a pair of experts whose interference gap $g_{AB}$ on a target concept is purely destructive (e.g., two math specialists whose combined outputs suffer from numeric interference). If collapsing this gap via a non-linear or high-rank merge results in **behavioral reinforcement** (improved accuracy) rather than the predicted unlearning, the causal link between the geometric residual and behavioral identity is broken.
