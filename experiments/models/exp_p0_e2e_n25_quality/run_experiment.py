#!/usr/bin/env python3
"""
P0.E2E: N=25 Quality Validation — Routing at Scale

Tests quality loss when combined logistic routing at N=25 (~89% accuracy)
drives adapter selection. Misrouted queries fall back to base model.

Kill criteria:
  K1486: GSM8K via 25-way routed adapter >= 68%
  K1487: HumanEval via 25-way routed adapter >= 46%
  K1488: MedMCQA via 25-way routed adapter >= 44%
  K1489: Max quality loss from routing <= 10pp vs oracle

Builds on:
  Finding #533: E2E N=10, 4pp max loss, Theorem 1 validated
  Finding #531: Combined logistic 88.8% at N=25
  Finding #508: 3 std LoRA adapters (math/code/medical)
"""
...
# 25 domains: 3 with adapters + 2 real (no adapter) + 20 MMLU subjects
# Same structure as Finding #531 (exp_p0_embedding_routing_n25)
REAL_NON_ADAPTER = OrderedDict([
    ("legal", ["professional_law", "jurisprudence", "international_law"]),
    ("finance", ["professional_accounting", "econometrics"]),
])
...
ALL_DOMAINS = ADAPTER_DOMAINS + list(REAL_NON_ADAPTER.keys()) + MMLU_SUBJECTS
assert len(ALL_DOMAINS) == 25, f"Expected 25 domains, got {len(ALL_DOMAINS)}"

IS_SMOKE = os.environ.get("SMOKE_TEST", "0") == "1"
N_EVAL = 5 if IS_SMOKE else 100
N_ROUTE_TRAIN = 20 if IS_SMOKE else 200
...
# ─────────────────────────────────────────────
# Phase 1: Train combined logistic router on 25 domains
# ─────────────────────────────────────────────
...
def load_routing_texts(n_per_domain):
    ...
    log("  Loading medical routing data...")
    path = hf_hub_download("openlifescienceai/medmcqa",
    ...
    # Legal: MMLU subjects
    log("  Loading legal routing data...")
    legal_texts = []
    for subject in REAL_NON_ADAPTER["legal"]:
        for split in ["test", "validation", "dev"]:
            try:
                path = hf_hub_download("cais/mmlu",
                    f"{subject}/{split}-00000-of-00001.parquet", repo_type="dataset")
                df = pd.read_parquet(path)
    ...
    # Finance: MMLU subjects
    log("  Loading finance routing data...")
    finance_texts = []
    for subject in REAL_NON_ADAPTER["finance"]:
        for split in ["test", "validation", "dev"]:
            try:
                path = hf_hub_download("cais/mmlu",
                    f"{subject}/{split}-00000-of-00001.parquet", repo_type="dataset")
    ...
    # 20 MMLU subjects
    log("  Loading 20 MMLU subject routing data...")
    for subject in MMLU_SUBJECTS:
        ...
        texts[subject] = domain_texts[:n_per_domain]
        if len(texts[subject]) < 50:
            log(f"    WARNING: {subject} only has {len(texts[subject])} texts")

    return texts


def phase_train_router():
    """Train combined TF-IDF + embedding logistic router for 25 domains."""
    from scipy.sparse import csr_matrix, hstack
    from sentence_transformers import SentenceTransformer
    from sklearn.feature_extraction.text import TfidfVectorizer
    from sklearn.linear_model import LogisticRegression

    log("\n[Phase 1] Training combined logistic router on 25 domains")
    t0 = time.time()
    ...
    # Adapter-domain routing accuracy
    ...
    log(f"  Non-adapter domain routing:")
    for d in ALL_DOMAINS:
        if d not in ADAPTER_DOMAINS:
...
def route_batch(router, queries):
    """Route a batch of queries through 25-way classifier."""
    from scipy.sparse import csr_matrix, hstack
...
# ─────────────────────────────────────────────
# Routed evaluation with base model fallback
# ─────────────────────────────────────────────

def eval_routed(router, benchmark_name, dataset, oracle_domain):
    """Evaluate with 25-way router. Adapter domains get adapter, others get base."""
    ...
    # Route all queries through 25-way classifier
    t_route = time.time()
...
def main():
    ...
    t_start = time.time()
    log("=" * 60)
    log("P0.E2E: N=25 Quality Validation — Routing at Scale")
    ...
    # Phase 1: Train 25-domain router
    ...
    # Phase 4: 25-way routed evaluation
    log("\n" + "=" * 60)
    log("Phase 4: 25-way combined logistic routed evaluation")
    log("=" * 60)
    ...
    alpha_math = routed_gsm8k["routing_accuracy_pct"] / 100
    alpha_code = routed_humaneval["routing_accuracy_pct"] / 100
    alpha_medical = routed_medmcqa["routing_accuracy_pct"] / 100

    predicted_gsm8k = alpha_math * oracle_gsm8k + (1 - alpha_math) * base_gsm8k
    predicted_humaneval = alpha_code * oracle_humaneval + (1 - alpha_code) * base_humaneval
    predicted_medmcqa = alpha_medical * oracle_medmcqa + (1 - alpha_medical) * base_medmcqa

    # Also compute a-priori predictions (before measuring routing)
    apriori_gsm8k = 0.95 * oracle_gsm8k + 0.05 * base_gsm8k
    apriori_humaneval = 0.88 * oracle_humaneval + 0.12 * base_humaneval
    apriori_medmcqa = 0.80 * oracle_medmcqa + 0.20 * base_medmcqa

    total_time = time.time() - t_start

    results = {
        "experiment": "exp_p0_e2e_n25_quality",
        "smoke": IS_SMOKE,
        "n_eval": N_EVAL,
        ...
        # A-priori predictions (using estimated routing accuracy)
        "apriori_gsm8k_pct": round(apriori_gsm8k, 1),
        "apriori_humaneval_pct": round(apriori_humaneval, 1),
        "apriori_medmcqa_pct": round(apriori_medmcqa, 1),
        ...
        # Comparison with N=10 (Finding #533)
        "n10_gsm8k_pct": 77.0,
        "n10_humaneval_pct": 56.0,
        "n10_medmcqa_pct": 54.0,
        "n10_max_loss_pp": 4.0,

        # Kill criteria
        "K1486_gsm8k": "PASS" if routed_gsm8k["accuracy_pct"] >= 68 else "FAIL",
        "K1487_humaneval": "PASS" if routed_humaneval["accuracy_pct"] >= 46 else "FAIL",
        "K1488_medmcqa": "PASS" if routed_medmcqa["accuracy_pct"] >= 44 else "FAIL",
        "K1489_max_loss": "PASS" if max_loss <= 10 else "FAIL",
    ...
    log("RESULTS SUMMARY")
    log("=" * 60)
    log(f"{'Benchmark':<12} {'Base':>8} {'Oracle':>8} {'Apriori':>8} {'T1-Pred':>8} {'Routed':>8} {'Loss':>8} {'N=10':>8}")
    log(f"{'GSM8K':<12} {base_gsm8k:>7.1f}% {oracle_gsm8k:>7.1f}% {apriori_gsm8k:>7.1f}% {predicted_gsm8k:>7.1f}% {routed_gsm8k['accuracy_pct']:>7.1f}% {gsm8k_loss:>7.1f}pp {77.0:>7.1f}%")
    log(f"{'HumanEval':<12} {base_humaneval:>7.1f}% {oracle_humaneval:>7.1f}% {apriori_humaneval:>7.1f}% {predicted_humaneval:>7.1f}% {routed_humaneval['accuracy_pct']:>7.1f}% {humaneval_loss:>7.1f}pp {56.0:>7.1f}%")
    log(f"{'MedMCQA':<12} {base_medmcqa:>7.1f}% {oracle_medmcqa:>7.1f}% {apriori_medmcqa:>7.1f}% {predicted_medmcqa:>7.1f}% {routed_medmcqa['accuracy_pct']:>7.1f}% {medmcqa_loss:>7.1f}pp {54.0:>7.1f}%")
    ...
    log("\nKILL CRITERIA:")
    log(f"  K1486 GSM8K routed >= 68%:      {results['K1486_gsm8k']} ({routed_gsm8k['accuracy_pct']:.1f}%)")
    log(f"  K1487 HumanEval routed >= 46%:   {results['K1487_humaneval']} ({routed_humaneval['accuracy_pct']:.1f}%)")
    log(f"  K1488 MedMCQA routed >= 44%:     {results['K1488_medmcqa']} ({routed_medmcqa['accuracy_pct']:.1f}%)")
    log(f"  K1489 Max routing loss <= 10pp:  {results['K1489_max_loss']} ({max_loss:.1f}pp)")

    log(f"\nDegradation from N=10 → N=25:")
    log(f"  GSM8K: {77.0:.1f}% → {routed_gsm8k['accuracy_pct']:.1f}% ({routed_gsm8k['accuracy_pct'] - 77.0:+.1f}pp)")
    log(f"  HumanEval: {56.0:.1f}% → {routed_humaneval['accuracy_pct']:.1f}% ({routed_humaneval['accuracy_pct'] - 56.0:+.1f}pp)")
    log(f"  MedMCQA: {54.0:.1f}% → {routed_medmcqa['accuracy_pct']:.1f}% ({routed_medmcqa['accuracy_pct'] - 54.0:+.1f}pp)")
