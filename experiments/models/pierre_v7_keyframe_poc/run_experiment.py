#!/usr/bin/env python3
"""Pierre v7: Keyframe adapter POC — deterministic arithmetic verifier.

The hypothesis: a ternary adapter can act as a VERIFIER, not just a domain nudge.
It should widen the logit gap between correct and incorrect arithmetic answers.

Approach:
  1. Generate arithmetic dataset: "X+Y=" → correct answer token
  2. Extract hidden states at the "=" position from base model
  3. Train a ternary classifier (STE) that predicts correct vs incorrect
  4. Inject as adapter: boosts correct answer logits, suppresses wrong ones
  5. Compose with domain adapter → verify non-interference (Grassmannian)

The adapter uses the same Grassmannian A-matrix framework as domain adapters.
A_verifier ⊥ A_domain by construction → they don't interfere.

Kill criteria:
  K745: Verifier accuracy < 60% on arithmetic (random = 50%)
  K746: Composing verifier + domain adapter degrades domain PPL > 10%
  K747: Training diverges (loss > 2x initial after 500 steps)
...
def phase_extract_features(data, model, tokenizer):
    """Forward pass through base model, extract hidden state at last token.

    Returns (features, labels) as numpy arrays.
    """
    log(f"  Extracting features for {len(data)} examples...")
    features = []
    labels = []
...
def main():
    ...
    k2 = all(abs(v) <= 10.0 for v in comp_results["degradation"].values())
    k3 = not train_stats["diverged"]

    results = {
        "experiment": "pierre_v7_keyframe_poc",
        ...
        "kill_criteria": {
            "K745": {"pass": k1, "value": eval_results["accuracy"], "threshold": 0.60},
            "K746": {"pass": k2, "value": max(abs(v) for v in comp_results["degradation"].values()),
                     "threshold": 10.0},
            "K747": {"pass": k3, "detail": f"initial={train_stats['initial_loss']}, final={train_stats['final_loss']}"},
        },
