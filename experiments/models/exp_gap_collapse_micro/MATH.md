...
Achieve a perfect linear merge ($g_{AB} \approx 0$) via weight-space alignment—such as through SVD-based rotation or DARE—that preserves the functional output of the specialist adapters. If the interference gap collapses without triggering the 50% unlearning of the target concept or the 5pp drop in OOD A/B discriminative accuracy, the hypothesis that the geometric gap is the functional locus of behavior is falsified.
