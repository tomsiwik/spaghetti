#!/usr/bin/env python3
"""
exp_hedgehog_procedural_adapter_refactor_full
=============================================

FULL of Hedgehog per-layer cos-sim distillation for procedural-refactor adapter.

Inherits MATH.md from parent exp_hedgehog_procedural_adapter_refactor_impl (F#784 PROVISIONAL).
Ports smoke-gate + enable_thinking=False mitigation pattern from politeness_full
(F#794+F#796 cross-validated).

Pre-registered KCs (canonical DB text — do not edit):
  K#2004 K1 structural: per-layer cos > 0.80 mean, > 0.70 worst, on N>=50 held-out refactor pairs
  K#2005 K2 target:     refactor quality auto-judge >= matched-rank token-space LoRA baseline
  K#2006 K3 target:     HumanEval pass@1 drop < 3pp vs base                  [DEFERRED to v2]
  K#2007 K4 target:     non-refactor gen-from-spec drop < 2pp vs base        [DEFERRED to v2]
  K#2008 K5 ablation:   NEUTRAL teacher retrain regresses K2 by >= 10pp      [DEFERRED to v2]
...
LR = 1e-4
WEIGHT_DECAY = 0.01
ENABLE_THINKING = False                  # F#790/F#794 cross-validated mitigation
GEN_MAX_TOKENS = 192 if IS_SMOKE else 256
...
def _apply_chat(tokenizer, msgs, tokenize: bool):
    """Chat template wrapper with enable_thinking=False graceful fallback."""
    try:
        return tokenizer.apply_chat_template(
            msgs, add_generation_prompt=True, tokenize=tokenize,
            enable_thinking=ENABLE_THINKING,
        )
    except TypeError:
        # tokenizer template doesn't support enable_thinking kwarg — fall back.
        return tokenizer.apply_chat_template(
            msgs, add_generation_prompt=True, tokenize=tokenize,
        )
...
def encode_prompt(tokenizer, system_prompt: str, user: str) -> mx.array:
    ...
    ids = _apply_chat(tokenizer, msgs, tokenize=True)
...
def generate_text(model, tokenizer, system_prompt: str, user_code: str,
    ...
    if system_prompt:
        msgs.append({"role": "system", "content": system_prompt})
    msgs.append({"role": "user", "content": f"Refactor this code:\n```python\n{user_code}\n```"})
    prompt_str = _apply_chat(tokenizer, msgs, tokenize=False)
...
# ──────────────────────────────────────────────
# Smoke validation gate (MATH.md §9; replicated from politeness_full / conciseness_full)
# ──────────────────────────────────────────────

def evaluate_smoke_gate(results: dict) -> dict:
    """Run pre-registered smoke gate. Returns dict with per-gate pass/fail
    and a `block_full_submission` boolean."""
    gates = {}
    # A1: Phase B loss converges
    pb = results.get("phase_b_student_train") or {}
    loss_first = pb.get("loss_first")
    loss_last = pb.get("loss_last")
    if loss_first is not None and loss_last is not None and loss_last > 0:
        gates["A1_loss_converges"] = "pass" if (loss_first / max(loss_last, 1e-9)) >= 2.0 else "fail"
    else:
        gates["A1_loss_converges"] = "fail"
    # A2: Phase C cos sanity
    k1 = (results.get("phase_c_k1_k2") or {}).get("K1") or {}
    cos = k1.get("mean_per_layer_cos")
    gates["A2_cos_sanity"] = "pass" if (cos is not None and cos >= 0.80) else "fail"
    # A3: K#2005 heuristic non-degenerate
    k2 = (results.get("phase_c_k1_k2") or {}).get("K2") or {}
    student_mean = k2.get("student_mean")
    gates["A3_heuristic_nondegen"] = "pass" if (student_mean is not None and student_mean >= 1.0) else "fail"
    # A4: enable_thinking=False produces non-thinking output (≥50 chars mean)
    samples = k2.get("samples") or []
    if samples:
        avg_len = sum(len(s.get("student_post", "")) for s in samples) / len(samples)
        gates["A4_thinking_off_nontrunc"] = "pass" if avg_len > 50 else "fail"
    else:
        gates["A4_thinking_off_nontrunc"] = "fail"
    # A5: adapter persists
    adapter_dir = EXPERIMENT_DIR / "adapters" / "hedgehog_refactor_r8"
    adapters_st = adapter_dir / "adapters.safetensors"
    config_json = adapter_dir / "adapter_config.json"
    gates["A5_adapter_persists"] = "pass" if (adapters_st.exists() and config_json.exists()) else "fail"
    # Block full submission if any gate failed
    failed = [k for k, v in gates.items() if v == "fail"]
    return {
        "gates": gates,
        "block_full_submission": len(failed) > 0,
        "failed_gates": failed,
    }
...
def main():
    ...
    results = {
        ...
        "enable_thinking": ENABLE_THINKING, "gen_max_tokens": GEN_MAX_TOKENS,
        "phase_0_dataset": None, "phase_b_student_train": None,
        "phase_c_k1_k2": {}, "phase_d_k3": None, "phase_d_k4": None, "phase_d_k5": None,
        "kc": {
            "K2004_per_layer_cos_gt_0_80": "untested",
            "K2005_refactor_judge_ge_baseline": "untested",
            "K2006_humaneval_drop_lt_3pp": "untested",
            "K2007_nonrefactor_drop_lt_2pp": "untested",
            "K2008_neutral_ablation_drop_ge_10pp": "untested",
        },
    ...
    try:
        ...
        if k1["mean_per_layer_cos"] is not None:
            results["kc"]["K2004_per_layer_cos_gt_0_80"] = (
    ...
    try:
        ...
        if k2["delta_pp"] is not None:
            if k2["judge"].startswith("claude") or k2["judge"].startswith("gpt"):
                ...
                # the canonical comparison vs matched-rank token-space LoRA is deferred to v2.
                results["kc"]["K2005_refactor_judge_ge_baseline"] = (
                    "pass" if k2["delta_pp"] >= 0.0 else "fail"
                )
            else:
                results["kc"]["K2005_refactor_judge_ge_baseline"] = "heuristic_only"
    except Exception as exc:
    ...
    results["phase_d_k3"] = {
        "deferred": True,
        "reason": "HumanEval pass@1 requires dedicated harness (mlx-evaluate or "
                  "custom pass@1 runner with N=164 standard set); scheduled v2.",
    }
    results["blockers"].append("K3 (HumanEval) DEFERRED to v2 — harness wiring required")
    results["phase_d_k4"] = {
        "deferred": True,
        "reason": "Non-refactor gen-from-spec drop requires curated 50-prompt "
                  "MBPP/HumanEval-style non-refactor slice; scheduled v2.",
    }
    results["blockers"].append("K4 (non-refactor) DEFERRED to v2 — curated 50-prompt set required")
    results["phase_d_k5"] = {
        "deferred": True,
        "reason": "NEUTRAL teacher ablation requires second 800-step training "
                  "run (REFACTOR_CATALOG_PROMPT removed); scheduled v2.",
    }
    results["blockers"].append("K5 (NEUTRAL ablation) DEFERRED to v2 — second training run required")
    ...
    # Run smoke gate (MATH.md §9)
    smoke_gate = evaluate_smoke_gate(results)
    results["smoke_gate"] = smoke_gate
    if IS_SMOKE and smoke_gate["block_full_submission"]:
        results["blockers"].append(
            f"smoke_gate FAILED: {smoke_gate['failed_gates']} — block pueue full submission"
        )

    # Verdict — never SUPPORTED in this iteration: K2 heuristic, K3+K4+K5 deferred.
    # Smoke caps at PROVISIONAL per MATH.md §9 verdict consistency.
