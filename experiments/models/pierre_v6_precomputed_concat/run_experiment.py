#!/usr/bin/env python3
"""Pierre v6: precomputed concatenated deltas — attention-only, QKV fused.

Three algebraic transforms, zero approximation:
  1. Attention-only: skip MLP (LoRA paper: sufficient) → 420→240
  2. Precompute ΔW = A@B offline (associativity) → 240→120
  3. QKV concatenation (shared input) → 120→60

Kill criteria:
  K742: Speed < 100 tok/s
  K743: Behavioral < 0.35
  K744: Memory > 6GB peak
"""
...
from pierre.v6 import (
    inject_precomputed, calibrate_router, route,
...
def phase_ppl():
    ...
    results = {"base": {}, "v6_single": {}, "v6_pierre": {}, "degradation": {}}
    ...
    for di, d in enumerate(DOMAINS):
        ...
        stats = inject_precomputed(model, skel, adapter, di, LORA_SCALE)
        results["v6_single"][d] = round(compute_ppl(model,tok,val[d]), 3)
        log(f"  v6_single/{d}: {results['v6_single'][d]} (dispatches={stats['dispatch_count']}, mem={stats['memory_mb']}MB)")
    ...
    for di, d in enumerate(DOMAINS):
        ...
        inject_precomputed(model, skel, adapter, ri, LORA_SCALE)
        results["v6_pierre"][d] = round(compute_ppl(model,tok,val[d]), 3)
        deg = (results["v6_pierre"][d]-results["v6_single"][d])/results["v6_single"][d]*100
        results["degradation"][d] = round(deg, 2)
        log(f"  pierre/{d}: {results['v6_pierre'][d]} (→{rd}, {deg:+.1f}%)")
        cleanup(model, tok, adapter)
...
def phase_latency():
    ...
    stats = inject_precomputed(model, skel, adapter, 0, LORA_SCALE)
    v6_tps, v6_mem = bench(model,tok,f"v6 precomputed ({stats['dispatch_count']} dispatches)")
    overhead = round((base_tps-v6_tps)/base_tps*100, 2)
    log(f"  Overhead: {overhead}%")
    log(f"  Delta memory: {stats['memory_mb']:.0f}MB")
    cleanup(model, tok, adapter)

    return {"base_tps": base_tps, "v6_tps": v6_tps, "overhead_pct": overhead,
            "base_peak_gb": base_mem, "v6_peak_gb": v6_mem,
            "delta_memory_mb": stats["memory_mb"],
            "dispatch_count": stats["dispatch_count"]}


def main():
    t0 = time.time()
    log("Pierre v6 — Precomputed Concatenated Deltas")
    log("=" * 60)
    mx.random.seed(SEED)
    ...
    k1 = r4["v6_tps"] >= 100.0
    k2 = r3["overall"] >= 0.35
    k3 = r4["v6_peak_gb"] <= 6.0

    results = {
        "experiment": "pierre_v6_precomputed_concat",
        "total_time_s": round(time.time()-t0, 1),
        "routing": r1, "ppl": r2, "behavioral": r3, "latency": r4,
        "kill_criteria": {
            "K742": {"pass": k1, "value": r4["v6_tps"], "threshold": 100.0},
            "K743": {"pass": k2, "value": r3["overall"], "threshold": 0.35},
            "K744": {"pass": k3, "value": r4["v6_peak_gb"], "threshold": 6.0},
        },
        "all_pass": k1 and k2 and k3,
        "comparison": {
            "v2_bf16_premerge": "47 tok/s, 0% overhead, 420 dispatches",
            "v3_bf16_sidepath": "73 tok/s, 48% overhead, 420 dispatches",
            "v5_ternary_sidepath": "77 tok/s, 45% overhead, 420 dispatches",
            "v6_precomputed": f"{r4['v6_tps']} tok/s, {r4['overhead_pct']}% overhead, {r4['dispatch_count']} dispatches",
    ...
    log(f"\nDispatch reduction: 420 → {r4['dispatch_count']} ({round((1-r4['dispatch_count']/420)*100)}% fewer)")
    log("\nKill criteria:")
