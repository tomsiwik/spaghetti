...
Usage:
    ...
    python tools/runpod_exec.py exec "nvidia-smi"       # run arbitrary command
...
pip install -q transformers'>=4.45.0' peft'>=0.13.0' datasets accelerate bitsandbytes scipy scikit-learn pyyaml tqdm 2>&1 | tail -3

python3 -c "
import os
from transformers import AutoModelForCausalLM, AutoTokenizer
hf = os.environ.get('HF_HOME', '/workspace/hf_cache')
AutoTokenizer.from_pretrained('Qwen/Qwen2.5-0.5B', cache_dir=hf)
AutoModelForCausalLM.from_pretrained('Qwen/Qwen2.5-0.5B', cache_dir=hf)
print('Qwen2.5-0.5B cached.')
"
...
def cmd_sync(args):
    ...
    cmd = [
        ...
        "--exclude", "experiments/macro/capsule_states",
        "--exclude", ".worktrees",
        "--exclude", "references",
        "-e", "ssh",
        f"{REPO_ROOT}/",
